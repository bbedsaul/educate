...
export class FirestoreProvider {
  ...
  docWithId$<T>(ref:  DocPredicate<T>): Observable<T> {
    return this.doc(ref).snapshotChanges().map(doc => {
      let data:T = doc.payload.data() as T;
  ...
  colWithIds$<T>(ref: CollectionPredicate<T>, queryFn?): Observable<any[]> {
    return this.col(ref, queryFn).snapshotChanges().map(actions => {
      return actions.map(a => {
        let data:{} = a.payload.doc.data() as any as object;
...
//    return this.afs.createId();
  ...
  atomicSave( docs: any[]) {
    ...
    docs.forEach(doc => {
      doc.item = firebase.firestore().doc(doc.id);
    });
 //   const currentTime = this.timestamp;
    const timestamp = this.timestamp

    docs.forEach(doc => {
      batch.update(doc.item, {...doc.data, "updatedAt": timestamp});
    });
    /// commit operations
    return batch.commit()
  }
